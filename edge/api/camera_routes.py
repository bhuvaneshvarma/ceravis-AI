...
def set_stream_profile(camera_id: str, body: dict | None = None):
    """Shape this camera's MAIN encoder — height and/or codec — on request only.

    Body: { "edgeId": "<this device>", "max_height": 1440, "codec": "H264" }
          Both shaping fields are optional; omit max_height to fall back to
          CAMERA_STREAM_MAX_HEIGHT. At least one is required.
    ...
    the same bar as PTZ from the cloud, not the looser installer-pad bar.
    ...
    tiles, the AI's input and the recordings. There is no way to shape some and
    ...
    `codec` exists because H.265 is a product-level fault, not a preference: no
    browser decodes HEVC over WebRTC (black live view) and our recordings are
    remuxed as-is (unplayable clips). Forcing H264 is the fix when the camera
    allows it.

    Nothing is inferred. The camera's supported resolutions and codecs are read
    first, the request is clamped into them, everything not named is preserved
    byte-for-byte, and the encoder is READ BACK afterwards. On top of that the
    live path is re-synced and the codec is OBSERVED off the real stream — this
    camera family reports "H264" over ONVIF while actually sending HEVC, so the
    camera's own read-back is a claim, not evidence. `observed_codec` is the
    truth; `after.codec` is only what the camera says."""
    body = body or {}
    ...
    codec = (field(body, "codec") or "").strip().upper() or None
    if max_height <= 0 and not codec:
        raise HTTPException(400, "max_height and/or codec required (or set "
    ...
    try:
        ...
        result = onvif_cam.shape_stream(profile.encoder_token,
                                        max_height=max_height or None, codec=codec)
    except OnvifError as exc:
    ...
    if result["changed"]:
        ...
        # consumer renegotiates against the new stream description.
        try:
            mediamtx_client.sync_live_path(cam)
        except MediaMTXError as exc:
            logger.warning("live path re-sync after profile change failed: %s", exc)
    result["observed_codec"] = _observe_codec(camera_id)

    logger.info("stream profile %s: %s", camera_id, result)
    problem = None
    if not result["accepted"]:
        problem = "camera did not accept the requested change"
    elif result["observed_codec"] and result["observed_codec"] != "h264":
        problem = (f"camera is really sending {result['observed_codec'].upper()} — "
                   f"browsers cannot play it, live view will be black and "
                   f"recordings unplayable")
    call_log.record("event", problem is None,
                    label=f"{camera_id} main stream "
                          f"{result['before']['resolution']} {result['before']['codec']}"
                          f" -> {result['after']['resolution']} "
                          f"{result['observed_codec'] or result['after']['codec']}",
                    error=problem)
    return {"camera_id": camera_id, **result}


def _observe_codec(camera_id: str, timeout: float = 6.0) -> str | None:
    """The codec the camera is REALLY sending, read off the live MediaMTX path.

    After an encoder change the path takes a moment to reconnect and publish its
    tracks, so this polls briefly rather than reading once and reporting None."""
    deadline = time.time() + timeout
    while time.time() < deadline:
        codec = mediamtx_client.path_codec(camera_id)
        if codec:
            return codec
        time.sleep(0.5)
    return None
