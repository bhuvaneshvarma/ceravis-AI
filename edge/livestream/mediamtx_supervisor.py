...
class MediaMTXSupervisor:
    ...
    def _write_config(self) -> None:
        ...
        lines = [
            ...
            "logLevel: info",
            # writeQueueSize is deliberately LEFT AT THE DEFAULT. It was raised
            # to 2048 on the theory that the AI reader was being dropped for
            # being too slow — measurement then showed the AI reader was healthy
            # all along (steady fps, no reconnects, nothing in the log), so the
            # change was fixing a problem that did not exist. A deeper queue is
            # not free either: it lets MediaMTX hold packets for a reader that
            # has fallen behind and then release them in a burst, which is worse
            # for a real-time consumer than dropping it and letting it redial.
            # Raise it only against a logged "reader is too slow".
            "",
            "api: yes",
        ...
        if cameras:
            ...
            for cam in cameras:
                ...
                # The camera's ONE connection: what the AI reads (loopback RTSP),
                # what the live links and UI pages play, and what gets recorded.
                # Quoted because the name carries a '/'.
                lines += [
                    f'  "{live}":',
                ...
                # pull) and re-encodes only the G.711 audio to AAC, so clips are
                # H.264+AAC, the combo every player and browser accepts.
                if audio:
                    out = record_path_name(cam)        # slash-free <cam>-aac
                    src = record_source_name(cam)      # the live path
                    lines += [
                        f"  {out}:",
