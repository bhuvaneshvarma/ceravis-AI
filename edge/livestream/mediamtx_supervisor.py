...
class MediaMTXSupervisor:
    ...
    def _write_config(self) -> None:
        ...
        lines = [
            ...
            "logLevel: info",
            # Per-reader packet queue. MediaMTX DISCONNECTS a reader that can't
            # keep up ("reader is too slow"), and our readers are not equal: a
            # WebRTC viewer just forwards packets, while the AI reader decodes
            # every frame and the recorder writes to disk. On a high-bitrate 4K
            # feed one keyframe is a burst of hundreds of packets, so the 512
            # default leaves barely a frame of slack — the AI session gets
            # dropped and reconnects while live view sails on, which is exactly
            # the "live is fine, the AI screens keep breaking" failure. 2048
            # packets (~3 MB per reader) absorbs the burst.
            "writeQueueSize: 2048",
            "",
            "api: yes",
